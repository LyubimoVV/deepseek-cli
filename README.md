...
Приложение поддерживает три стратегии управления контекстом для оптимизации токенов:

### 1. Без управления (None)
- Полная история диалога
- Используйте для коротких сессий (< 50 сообщений)

### 2. Суммаризация (Compression) - по умолчанию
- Автоматическое создание summary старых сообщений
- Параметры: `keepMessagesCount` (количество последних сообщений), `summaryInterval` (интервал создания summary)
- Используйте для длинных сессий с важным контекстом

### 3. Скользящее окно (Sliding Window)
- Хранятся только последние N сообщений
- Параметр: `windowSize` (размер окна, от 1 до 100)
